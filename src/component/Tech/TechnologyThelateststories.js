...
export function TechnologyThelateststories(props){
    ...
    return(
        <div className="tech_latest_stories">
          <h1 className="tech_stores_h1">Latest Stories</h1>
           ...
           {props.data.filter((item)=>item.categorey==="Latest Stories" && item.name==='Technology').map((d,index)=>{
                return(
                    ...
                    {token?<Link to={d.path} className="remove_underline"><p>{d.title}</p></Link>:<Link to='/signuppage' className="remove_underline"><h3>{d.title}</h3></Link>}
                    <p>{d.publishedAt}</p>
                    </div>  
                )
             })}
